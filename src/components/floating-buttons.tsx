...
const btnClass =
  "flex items-center justify-center w-14 h-14 rounded-full bg-[#ebebeb] dark:bg-[#1f1f1f] backdrop-blur-md text-[#737373] transition-colors duration-200 hover:bg-[#e0e0e0] dark:hover:bg-[#2a2a2a] hover:text-[#363636] dark:hover:text-[#ffffff]";

export function FloatingButtons({ showBack = false, showSocials = true }: { showBack?: boolean; showSocials?: boolean }) {
  const router = useRouter();

  return (
    <div className="fixed bottom-[40px] left-0 right-0 flex items-center justify-center gap-[10px] px-6 z-50">
      {showBack && (
        <button
          onClick={() => router.push("/")}
          aria-label="Back"
